...
/*   Updated: 2022/02/14 15:13:12 by ancarol3         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int	index;// varrer o array de entrada
...
int	ft_initialize_process(int *input, int **init, int **work, int n)
{
	...
	index = 0;
	offset = 0;
	while (index < FT_OBSERVER_LINES)// oberver_lines vai ser sempre 4
	{
		observers[index] = ft_allocate_array(n); // allocar espaços para os observadores 
		ft_fill_array(observers[index], input, offset, n);//e vai preencher o array
		offset += n;
		index++;
	}
	// vai preencher os obvios,os mais basicos
...
int	ft_allocation_step(char *input, int input_size)
{
	...
	if (ft_validate_input_size(input_size) == 0)
		return (0);
	ft_parse_input(input, input_array); // vai
